...
const explorerChains = {
    ...
    eth: { url: 'https://ethbook.guarda.com/api', isSteemFork: false, isEVM: true, evmDecimals: 18 },
    etc: {
...
// --- Solana Pay (native Solana request/verify via @solana/pay; no API key) ---
// A request/verify checkout rail (not an address-watch like the `sol` chain above):
//   createPayment({ recipient, amount, splToken?, reference?, label?, message?, memo? })
//     -> { url, reference }  — a `solana:` URL/QR for the payer; KEEP the `reference`.
//   existsTransaction(recipient, amount, timestamp, reference)
//     -> verifies that order via findReference + validateTransfer (the `reference`
//        uniquely identifies the order, so same-amount payments never collide).
// Free: needs only a Solana RPC (public default is rate-limited — set
// NEKOPAY_SOLANAPAY_EXPLORER_URL to a Helius/QuickNode endpoint for volume). Requires the
// optional deps @solana/pay + @solana/web3.js + bignumber.js.
const solanaPayGateways = {
    solanapay: { url: 'https://api.mainnet-beta.solana.com/', isSolanaPay: true, solDecimals: 9, verified: true }
};
...
const fiatGateways = {
    ...
    // Fortumo was acquired by Boku (2020) and the brand consolidated — legacy API.
    fortumo: { url: 'https://api.fortumo.com/', isFortumo: true, gatewayType: 'fortumo', verified: false, deprecated: true },
