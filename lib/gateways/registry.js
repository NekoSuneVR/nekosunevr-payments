...
const systems = {};
// Every system carries explicit boolean status flags; callers/loops override via meta.
const register = (key, meta) => {
    systems[moduleName(key)] = {
        module: moduleName(key),
        key,
        verified: true,
        deprecated: false,
        explorerOffline: false,
        ...meta
    };
};
...
Object.keys(explorerChains).forEach((key) => {
    ...
    const category = config.isEVM ? 'crypto-evm'
        : config.isTron ? 'crypto-tron'
        : config.isSolana ? 'crypto-solana'
        : 'crypto-utxo';
    register(key, { category, chain: key, free: true, crypto: true, requiresKeys: [], verified: config.verified !== false, explorerOffline: !!config.explorerOffline });
});
Object.keys(solanaPayGateways).forEach((key) => {
    const config = solanaPayGateways[key];
    // Free Solana checkout rail (@solana/pay): no API key, just an RPC endpoint.
    register(key, { category: 'crypto-solana', chain: key, free: true, crypto: true, requiresKeys: [], verified: config.verified !== false });
});
...
// Status lookups. Every system carries boolean `verified`/`deprecated`/`explorerOffline`
// (false unless explicitly flagged in config).
const listUnverified = () => listAll().filter((s) => s.verified === false).map((s) => s.module);
const listDeprecated = () => listAll().filter((s) => s.deprecated).map((s) => s.module);
const listExplorerOffline = () => listAll().filter((s) => s.explorerOffline).map((s) => s.module);
