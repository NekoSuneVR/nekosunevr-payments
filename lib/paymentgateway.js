...
class PaymentAPI {
    constructor(config) {
        ...
        if (this.isWooCommerce) {
            this.woocommerceAuth = Buffer.from(`${this.woocommerceConsumerKey}:${this.woocommerceConsumerSecret}`).toString('base64');
        }
    }
    ...
    async fetchData(endpoint, options = {}) {
        try {
            const headers = {
                ...
                ...(this.isWooCommerce && { Authorization: `Basic ${this.woocommerceAuth}` }),
                ...(options.headers || {})
            };
    ...
    async createXsollaPaymentToken(paymentData) {
        ...
        try {
            const response = await this.fetchPostData('merchant/projects/{project_id}/payment/token', {
                user: {
    ...
    async createWooCommerceOrder(orderData) {
        ...
        try {
            const response = await this.fetchPostData('orders', {
                payment_method: orderData.payment_method || 'skrill',
                payment_method_title: orderData.payment_method_title || 'Skrill',
                set_paid: false,
                billing: {
                    email: orderData.email || ''
                },
                line_items: [{
                    name: orderData.description || 'Order',
                    quantity: 1,
            ...
            throw new Error(`Failed to create WooCommerce order: ${error.message}`);
        }
    }
            ...
            try {
                ...
                for (const txid of transactions) {
                    ...
                    if (this.isCoinbase) {
                        ...
                        const md5Signature = crypto
                            .createHash('md5')
                            .update(`${transaction.transaction_id}${transaction.amount}${transaction.currency}${this.skrillSecretWord}`)
                            .digest('hex');
                        isMatch = transaction.amount === amount &&
                        ...
                        isMatch = parseFloat(transaction.total) === amount &&
                                 ...
                                 (!memo || transaction.order_notes === memo);
                    } else {
...
Object.keys(chainConfigs).forEach(chain => {
    exports[`${chain.toUpperCase()}Module`] = class extends ChainModule {
        constructor(domain) {
            super(chain, chainConfigs[chain], domain);
        }
    };
});
