...
const querySteemForkRpc = async (rpcKind, urls, handler) => {
    let lastError = null;
...
class PaymentAPI {
    ...
    hasDirectAntelopeApi() {
        return !!(this.antelopeHistoryUrl && this.assetSymbol);
    }

    async getDirectSteemForkHistory(address) {
        ...
        return querySteemForkRpc(this.rpcKind, this.rpcUrls, async (client) => {
            ...
            return callClientMethod(client.api, 'getAccountHistory', [address, -1, 1000]);
        });
    }
    ...
    async findDirectSteemForkTransaction(address, amount, memo = null) {
    ...
    async getDirectAntelopeAccount(address) {
        if (!this.antelopeChainUrl) return null;
        const response = await axios.post(
            this.antelopeChainUrl,
            { account_name: address },
            {
                headers: {
                    'Content-Type': 'application/json',
    ...
    async getDirectAntelopeBalances(address) {
        if (!this.antelopeBalancesUrl) return null;
        const response = this.antelopeBalancesMethod === 'POST'
            ? await axios.post(
                this.antelopeBalancesUrl,
            ...
            : await axios.get(`${this.antelopeBalancesUrl.replace(/\/$/, '')}/${address}`, {
                headers: {
                    'User-Agent': 'Mozilla/5.0'
                }
            });
    ...
    async getDirectAntelopeTransaction(txid) {
        if (!this.antelopeTxUrl) return null;
        ...
        const response = await axios.get(this.antelopeTxUrl, {
            params: { id: txid },
            headers: {
                'User-Agent': 'Mozilla/5.0'
            }
    ...
    async findDirectAntelopeTransaction(address, amount, memo = null) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (this.api.hasDirectAntelopeApi()) {
                    const transaction = await this.api.findDirectAntelopeTransaction(address, amount, memo);
                ...
                if (this.api.hasDirectSteemForkRpc()) {
                    const transaction = await this.api.findDirectSteemForkTransaction(address, amount, memo);
                    const confirmations = Number(transaction?.confirmations || 0);
                    if (!transaction || confirmations < minimumConfirmations) {
                        return resolve({
                            exists: false,
