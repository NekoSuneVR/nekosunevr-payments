...
const solanaPay = require('./solanaPay');
const {
    normalizeAddress,
...
class ChainModule {
    constructor(chain, config, domain = null) {
        ...
        // Preserve case for Solana (base58 mints are case-sensitive; covers both the
        // address-watch `isSolana` chain and the `isSolanaPay` checkout module); lowercase
        // EVM/Tron contracts.
    ...
    async existsTransaction(address, amount, timestamp, memo = null, minimumConfirmations = 0) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // Solana Pay: verify by the order's `reference` pubkey (passed as `memo`),
                // not by scanning an address. findReference + validateTransfer confirm the
                // recipient/amount/spl-token match the request we issued.
                if (this.isSolanaPay) {
                    const reference = memo; // the `reference` returned by createPayment
                    const connection = solanaPay.getConnection(this.api.explorerUrl);
                    const result = await solanaPay.findAndValidatePayment(connection, {
                        recipient: address,
                        amount,
                        reference,
                        splToken: this.tokenContract
                    });
                    if (!result.found) {
                        return resolve({ exists: false, txid: '', conf: '' });
                    }
                    const confirmations = await solanaPay.getConfirmations(connection, result.signature);
                    if (Number(confirmations || 0) < minimumConfirmations) {
                        return resolve({ exists: false, txid: result.signature, conf: confirmations });
                    }
                    return resolve({ exists: true, txid: result.signature, conf: confirmations });
                }
    ...
    async createPayment(chargeData) {
        if (this.isSolanaPay) return this.createSolanaPayURL(chargeData);
    ...
    // Solana Pay: build a `solana:` request URL + a unique per-order reference key.
    // chargeData: { recipient|address|to, amount, splToken?, reference?, label?, message?, memo? }
    // Returns { url, reference }. Persist `reference` and pass it back to existsTransaction
    // (as the memo/reference argument) to verify the payment later.
    createSolanaPayURL(chargeData = {}) {
        if (!this.isSolanaPay) throw new Error('Solana Pay URL creation only supported for the Solana Pay module');
        return solanaPay.createSolanaPayRequest({
            recipient: chargeData.recipient || chargeData.address || chargeData.to,
            amount: chargeData.amount,
            splToken: chargeData.splToken || chargeData.spl_token || this.tokenContract,
    ...
    // Solana Pay: render a created request URL as a QR code object (@solana/pay createQR).
    // In Node, call `.getRawData('png')` on the result to get a PNG Buffer.
    createSolanaPayQR(url, size, background, color) {
        if (!this.isSolanaPay) throw new Error('Solana Pay QR creation only supported for the Solana Pay module');
        return solanaPay.createSolanaPayQR(url, size, background, color);
    }
